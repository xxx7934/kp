...
Galv.QUEST.file = {};
Galv.QUEST.file.getString = function(filePath, language) {
	...
	request.onload = function() {
		if (request.status < 400) {
			if(language === 0)
				Galv.QUEST.createJPQuests(request.responseText);
			if(language === 1)
				Galv.QUEST.createENQuests(request.responseText);
		}
	};
	request.send();
};

Galv.QUEST.createENQuests = function(string) {
	...
	for (var i = 0; i < lines.length; i++) {
		if (lines[i][0] == '<') {
			if (lines[i].contains('</quest>')) {
				...
				if (qId) {
					bIndex = Number(qId[1]);
					Galv.QUEST.entxt[bIndex] = {};
					Galv.QUEST.entxt[bIndex].desc = [];
					
					var s = qId[2].split('|');
					Galv.QUEST.entxt[bIndex].name = s[0] || '???';
					Galv.QUEST.entxt[bIndex].difficulty = s[1] || '???';
					Galv.QUEST.entxt[bIndex].category = s[2] || 0;
					
					record = true;
				}
			} 
		} else if (record) {
			Galv.QUEST.entxt[bIndex].desc.push(lines[i]);
		}
	};
};

Galv.QUEST.createJPQuests = function(string) {
	var lines = string.split("\n");
	var bIndex = 0;
	var record = false;
	Galv.QUEST.jptxt = {};
	...
	for (var i = 0; i < lines.length; i++) {
		if (lines[i][0] == '<') {
			if (lines[i].contains('</quest>')) {
				...
				if (qId) {
					bIndex = Number(qId[1]);
					Galv.QUEST.jptxt[bIndex] = {};
					Galv.QUEST.jptxt[bIndex].desc = [];
					
					var s = qId[2].split('|');
					Galv.QUEST.jptxt[bIndex].name = s[0] || '???';
					Galv.QUEST.jptxt[bIndex].difficulty = s[1] || '???';
					Galv.QUEST.jptxt[bIndex].category = s[2] || 0;
					
					record = true;
				}
			} 
		} else if (record) {
			Galv.QUEST.jptxt[bIndex].desc.push(lines[i]);
		}
	};
};
...
Galv.QUEST.fileName = function() {
	if (!Galv.QUEST.entxt) {
		...
		//Galv.QUEST.entxt = {};
		Galv.QUEST.file.getString(folder + filename + "EN.txt", 1);
	};
	if (!Galv.QUEST.jptxt) {
		var filename = PluginManager.parameters('Galv_QuestLog')["File"];;
		var folder = PluginManager.parameters('Galv_QuestLog')["Folder"];
		if (folder !== "") folder = folder + "/";
		//Galv.QUEST.jptxt = {};
		Galv.QUEST.file.getString(folder + filename + "JP.txt", 0);
	};
}();
...
var catsEN = RemGALVQuestCategoriesEN.split(',');
var catsJP = RemGALVQuestCategoriesJP.split(',');
Galv.QUEST.categoriesEN = []; Galv.QUEST.categoriesJP = [];
for (var i = 0; i < catsEN.length; i++) {
	var dataEN = catsEN[i].split("|");
	var dataJP = catsJP[i].split("|");
	Galv.QUEST.categoriesEN[i] = {id:i,name:dataEN[0],color:dataEN[1]};
	Galv.QUEST.categoriesJP[i] = {id:i,name:dataJP[0],color:dataJP[1]};
}
...
Galv.QUEST.create = function(id) {
	if (!$gameSystem._quests.quest[id] && Galv.QUEST.jptxt[id]) {
...
Game_System.prototype.initialize = function() {
	...
	for (var i = 0; i < Galv.QUEST.categoriesEN.length; i++) {
...
Game_Quest.prototype.desc = function() {
	if(TextManager.isJapanese)
		return Galv.QUEST.jptxt[this._id].desc.slice(2);  
	else if(TextManager.isEnglish)
		return Galv.QUEST.entxt[this._id].desc.slice(2); 
	// returns the description without the first 2 lines (which is objectives and resolutions)
};

Game_Quest.prototype.name = function() {
	if(TextManager.isJapanese)
		return Galv.QUEST.jptxt[this._id].name;
	else if(TextManager.isEnglish)
		return Galv.QUEST.entxt[this._id].name;
};

Game_Quest.prototype.difficulty = function() {
	if(TextManager.isJapanese) {
		var diff = Galv.QUEST.jptxt[this._id].difficulty;
		var text = "";
		for(;diff>0;diff--) text += "★";
		return text;
	}
	else if(TextManager.isEnglish) {
		var diff = Galv.QUEST.entxt[this._id].difficulty;
		var text = "";
		for(;diff>0;diff--) text += "★";
		return text;
	}
...
Game_Quest.prototype.category = function() {
	if(TextManager.isJapanese)
		return Galv.QUEST.jptxt[this._id].category;
	else if(TextManager.isEnglish)
		return Galv.QUEST.entxt[this._id].category;
};
...
Game_Quest.prototype.objectives = function() {
	...
	if(TextManager.isJapanese)
		array = Galv.QUEST.jptxt[this._id].desc[0].split(Galv.QUEST.sep);
	else if(TextManager.isEnglish)
		array = Galv.QUEST.entxt[this._id].desc[0].split(Galv.QUEST.sep);
...
Game_Quest.prototype.resolutions = function() {
	...
	if(TextManager.isJapanese)
		array = Galv.QUEST.jptxt[this._id].desc[1].split(Galv.QUEST.sep);
	else if(TextManager.isEnglish)
		array = Galv.QUEST.entxt[this._id].desc[1].split(Galv.QUEST.sep);
...
Window_QuestCategory.prototype.makeCommandList = function() {
	...
	this.addCommand(TextManager.RemGALVQuestActive,    'active');
    this.addCommand(TextManager.RemGALVQuestCompleted,  'completed');
    ...
    if (item) {
		...
		if (item.categoryTitle != undefined) {
			...
			if(TextManager.isEnglish)
				cat = Galv.QUEST.categoriesEN[item.categoryTitle];
			else if(TextManager.isJapanese)
				cat = Galv.QUEST.categoriesJP[item.categoryTitle];
			var txt = cat.name;
...
Window_QuestInfo.prototype.drawQuest = function(quest) {
	...
	this.drawText(TextManager.RemGALVQuestDetails,0,y,w);
	...
	if (objs.length > 0) {
		...
		this.drawText(TextManager.RemGALVQuestObjectives,0,y,w);
		this.changeTextColor(this.normalColor());
		y += line + 10;
